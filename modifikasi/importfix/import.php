...
if (isset($_POST['doImport'])) {
  ...
  if (!isset($_POST['process'])) {
    ...
    if (!$csv_upload->getUploadStatus()) {
      $upload_error = trim((string) $csv_upload->getError());
      if ($upload_error === '') {
        $upload_error = __('Upload failed! File type not allowed or the size is more than') . ' ' . ($sysconf['max_upload'] / 1024) . ' MB';
      }

      toastr($upload_error)->error(__('Import Tool'));
      exit;
    }
    ...
    $start_time = time();
    // Keep running even if the browser is closed mid-import
    ...
    try {
      ...
      $reader->each(formatter: function (&$field, $row, $index) use ($dbs) {
      ...
      }, processor: function ($reader, $row) use ($dbs, $pdo, $sysconf, $indexer, $lineNumber, &$inserted_row, $state) {
        ...
        $authors = $fields[15] ?? null;
        $subjects = $fields[16] ?? null;
        $items = $fields[17] ?? null;
        ...
        $insert_success = $state->execute($fields);

        if ($insert_success) {
          $biblio_id = (int) $pdo->lastInsertId();

          if ($biblio_id === 0) {
            $lookup = $pdo->prepare('SELECT biblio_id FROM biblio WHERE title = ? ORDER BY biblio_id DESC LIMIT 1');
            $lookup->execute([$fields[0]]);
            $biblio_id = (int) ($lookup->fetchColumn() ?: 0);
          }

          if ($biblio_id === 0) {
            throw new RuntimeException(
              str_replace('{title}', $fields[0] ?? '?', __('Unable to resolve imported bibliographic record ID : {title}'))
            );
          }
      ...
      // Reset session
      unset($_SESSION['csv']);
      $errorMessage = json_encode((string) $e->getMessage(), JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT);
      exit(<<<HTML
      <script>
    ...
    $label = json_encode(
      str_replace(['{row_count}', '{time_to_finish}'], [$inserted_row, $import_time_sec], __('Success imported <strong>{row_count}</strong> title in <strong>{time_to_finish}</strong> second')),
